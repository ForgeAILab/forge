...
impl CreateDomainEvent {
    ...
    pub fn task_transition(
        ...
        // State names are workflow identifiers; truncating them would detach
        // the ledger record from its immutable semantic snapshot.
        let from_state = from_state.as_ref().to_owned();
        let to_state = to_state.as_ref().to_owned();
...
/// Whether a Task interruption calls for autonomous recovery attention.
/// Manual stops retain their user-facing recovery controls but do not invite
/// an Agent to undo the user's stop. Use this for both event creation and
/// current-state wake admission so the two boundaries cannot disagree.
pub fn task_interruption_requires_intervention(
...
pub trait ProjectRepo: Send + Sync {
    ...
    /// Public system Projects, owned Projects, and explicit memberships only.
    ...
    /// Apply account visibility before pagination and counts.
...
/// Main-agent sub-agent runs ("inquiries"). This is a run log the Main
/// session can inspect and cancel, not a work item -- no repo, no task
/// flow, no state machine, and no retry/assignment/dependency/milestone/
/// review concept. `id`/timestamps are generated by the store, matching the
/// narrow set of other mutation methods in this crate whose input carries
/// no timestamp field.
#[async_trait]
pub trait AgentInquiryRepo: Send + Sync {
    ...
    /// Keyset-paginated, newest first. Mirrors `ProjectHookRunRepo::list_for_project`'s
    /// cursor idiom: an opaque `(created_at, id)` cursor, decoded/encoded internally.
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CompleteAgentInquiry {
    pub id: String,
    pub expected_version: i64,
