-- One identity may fill more than one role on the same Task. Keep the
-- canonical Task id in scope_id for authorization, memory, and tool routing,
-- while making the session scope unique per Task role so worker and reviewer
-- capabilities cannot collide or be reused across authority classes.
