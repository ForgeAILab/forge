-- The Project Agent had no way to say "I verified this myself".
--
-- Acceptance checks could only be `manual`, so the Agent described a check as
-- agent-verifiable and then still handed the observation to the user. With
-- `project.validation` in place, `task_validation` is the right source kind for
-- anything the Agent can settle by exercising the delivered software, and the
-- protocol now says so. Revision @6 is that guidance; the body is otherwise
-- identical to @5.
...
SELECT
    'forge.project.orchestration/v1@6', operating_skill_id, skill_key, 6,
...
SET current_revision_id = 'forge.project.orchestration/v1@6',
    version = version + 1,
    updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')
WHERE id = 'forge.project.orchestration/v1'
  AND current_revision_id IS NOT 'forge.project.orchestration/v1@6';

UPDATE project_agent_binding
SET operating_skill_revision_id = 'forge.project.orchestration/v1@6'
WHERE operating_skill_revision_id = 'forge.project.orchestration/v1@5';
