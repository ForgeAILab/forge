-- The execution baseline is gone; revision @9 still taught the Agent to use it.
--
-- @9 told the Project Agent to propose an execution baseline, to treat the
-- baseline as part of effective Project state, and -- worst -- that "Recording
-- any result requires the Project's active approved execution baseline; when
-- none is active, say so and ask the user to approve one rather than reporting
-- the delivery as validated." That instruction turned a Project that had
-- delivered every acceptance check into one that asked its user to approve a
-- second artifact before its work could be called validated.
--
-- The baseline no longer exists. Readiness is the approved Charter plus the
-- milestone's own acceptance matrix: every required check has a current passing
-- result and required evidence is attached. @10 removes every baseline
-- instruction so the Agent stops asking for an artifact the system cannot
-- create.
...
SELECT
    'forge.project.orchestration/v1@10', operating_skill_id, skill_key, 10,
...
WHERE id = 'forge.project.orchestration/v1@9';

UPDATE operating_skill
SET current_revision_id = 'forge.project.orchestration/v1@10',
    version = version + 1,
    updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')
WHERE id = 'forge.project.orchestration/v1'
  AND current_revision_id IS NOT 'forge.project.orchestration/v1@10';

UPDATE project_agent_binding
SET operating_skill_revision_id = 'forge.project.orchestration/v1@10'
WHERE operating_skill_revision_id = 'forge.project.orchestration/v1@9';
