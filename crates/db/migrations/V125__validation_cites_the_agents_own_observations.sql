-- @13 told the Agent that integrated verification is its own work; nothing
-- made it so. On a delivery wake the Agent recorded five passes in fifty
-- seconds from the reviewer's report, with no build in its checkout. @14 names
-- the mechanism the server now enforces: a task_validation pass or fail must
-- cite the observation ids of commands the Agent ran itself in checkout/,
-- newer than the delivered Task. A Task's report is narration.
...
SELECT
    'forge.project.orchestration/v1@14', operating_skill_id, skill_key, 14,
...
WHERE id = 'forge.project.orchestration/v1'
  AND current_revision_id IS NOT 'forge.project.orchestration/v1@14';

UPDATE project_agent_binding
SET operating_skill_revision_id = 'forge.project.orchestration/v1@14'
WHERE operating_skill_revision_id = 'forge.project.orchestration/v1@13';
