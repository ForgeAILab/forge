-- The fresh-bootstrap gap @12 left open: the check-source preference lived
-- only in the on-demand milestones section, so a Charter-handoff turn that
-- never read it authored machine-verifiable behaviors as `manual` checks.
-- That parks readiness on user attestation for work the Agent is supposed to
-- verify itself, inverting the verification doctrine at the very first
-- definition. @13 pulls the one-line rule into the resident core: author
-- acceptance checks the way you will settle them.
...
SELECT
    'forge.project.orchestration/v1@13', operating_skill_id, skill_key, 13,
...
WHERE id = 'forge.project.orchestration/v1@12';
...
WHERE id = 'forge.project.orchestration/v1'
  AND current_revision_id IS NOT 'forge.project.orchestration/v1@13';

UPDATE project_agent_binding
SET operating_skill_revision_id = 'forge.project.orchestration/v1@13'
WHERE operating_skill_revision_id = 'forge.project.orchestration/v1@12';
