-- The Main Agent's scratch directory had nowhere to live.
--
-- A Main Agent Chat now owns a workspace: an account scratch directory it
-- keeps findings and notes in, and that the ephemeral inquiry sub-agents it
-- dispatches work inside. The inquiry sub-agent itself runs under an Account
-- scope holding the same directory. Two constraints written when
-- `project_verify` was the only self-owned workspace refused both:
--
--   CHECK constraint failed: workspace_access IN
--       ('deny','task_read','task_write','project_verify')
--   only a project_verify scope carries its own workspace path
--
-- `account_scratch` is admitted for exactly two shapes, and the compound
-- CHECK says which: an 'account' row (what an inquiry runs under), and an
-- 'agent_chat' row with no Project -- that is, a Main Chat. A Project Agent
-- Chat therefore cannot hold one, which is the same Main-only rule the tool
-- composition enforces, restated where the data lives.
--
-- `account_scratch` is deliberately not a repository: no clone, no worktree,
-- no remote. Widening this admits a scratch directory, never a checkout.
...
DROP TRIGGER agent_context_scope_workspace_path_guard_insert;
DROP TRIGGER agent_context_scope_workspace_path_guard_update;
...
CREATE TABLE agent_context_scope_new (
    ...
    workspace_access    TEXT NOT NULL DEFAULT 'deny'
                            CHECK (workspace_access IN (
                                'deny', 'task_read', 'task_write',
                                'project_verify', 'account_scratch'
                            )),
    workspace_path      TEXT,
    ...
    CHECK (
        -- An 'account' row is either an ordinary denied account session or
        -- the scratch-holding scope an inquiry sub-agent runs under.
        (scope_type = 'account' AND project_id IS NULL AND room_id IS NULL
            AND task_id IS NULL AND task_role IS NULL
            AND workspace_access IN ('deny', 'account_scratch'))
        ...
        -- Project and carries `account_scratch`. Requiring the Project to be
        -- absent is what keeps a scratch directory off a Project Chat.
        OR (scope_type = 'agent_chat' AND room_id IS NULL AND task_id IS NULL
            ...
            AND (
                ...
                OR (workspace_access = 'account_scratch' AND project_id IS NULL)
            ))
    )
);
...
INSERT INTO agent_context_scope_new (
    ...
    task_role, workspace_access, workspace_path, authority_json, version,
...
-- A path is meaningful only where the scope owns its workspace. A Task scope
-- keeps the `workspace` table as its authority, and a denied scope has none.
CREATE TRIGGER agent_context_scope_workspace_path_guard_insert
BEFORE INSERT ON agent_context_scope
WHEN NEW.workspace_path IS NOT NULL
    AND NEW.workspace_access NOT IN ('project_verify', 'account_scratch')
BEGIN
    SELECT RAISE(
        ABORT,
        'only a project_verify or account_scratch scope carries its own workspace path'
    );
END;
