-- The Project Agent's resident protocol shrinks to an authority core plus a
-- doctrine index. Revision @10 carried every operating-doctrine section
-- (research, documents, scope change, tasks, milestones, release) inline, so
-- each Project Chat turn paid ~20KB of fixed prompt before the conversation
-- began — on small provider profiles the required content alone exceeded the
-- context window and no amount of LCM compaction could recover the chat.
--
-- @11 keeps the immutable contract resident (mission, startup, authority and
-- scope, standing invariants, autonomous drive, communication, refusal) and
-- moves the detailed doctrine behind the new `skill.section` read operation.
-- The approved Charter likewise stops being inlined into the handoff packet;
-- the Agent reads it on demand through the new `project.charter` read.
...
SELECT
    'forge.project.orchestration/v1@11', operating_skill_id, skill_key, 11,
...
',
    ...
    'e90cc95f544c08299b5f94762df8ca5017bf158c494454e95dfeebf11fd5ce74',
...
SET current_revision_id = 'forge.project.orchestration/v1@11',
    version = version + 1,
    updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')
WHERE id = 'forge.project.orchestration/v1'
  AND current_revision_id IS NOT 'forge.project.orchestration/v1@11';

UPDATE project_agent_binding
SET operating_skill_revision_id = 'forge.project.orchestration/v1@11'
WHERE operating_skill_revision_id = 'forge.project.orchestration/v1@10';
