-- The Project Agent now has a workspace, and revision @8 told it otherwise.
--
-- @8 said "You cannot observe software behaviour yourself", which was true when
-- a Project Agent session had no filesystem and no process. It now runs inside
-- its own workspace: `forge/` for the memory and documents it keeps, and
-- `checkout/` -- a disposable detached worktree -- for exercising the delivered
-- software. An Agent following @8 would refuse the capability it has.
--
-- @9 states the workspace, keeps implementation in Tasks (an edit in `checkout/`
-- reaches nobody), points at `project.observations` for reading what Task runs
-- reported, and relaxes the citation rule to match: name a Task when a Task
-- observed, omit it when you observed, never name one that did not run.
...
SELECT
    'forge.project.orchestration/v1@9', operating_skill_id, skill_key, 9,
...
',
    ...
    'cb0cbeac81d85dfb3f9747267cd73d5bf1c52cb90af0861c8bece0123738bc36',
    'system', strftime('%Y-%m-%dT%H:%M:%fZ','now')
FROM operating_skill_revision
WHERE id = 'forge.project.orchestration/v1@8';

UPDATE operating_skill
SET current_revision_id = 'forge.project.orchestration/v1@9',
    version = version + 1,
    updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')
WHERE id = 'forge.project.orchestration/v1'
  AND current_revision_id IS NOT 'forge.project.orchestration/v1@9';

UPDATE project_agent_binding
SET operating_skill_revision_id = 'forge.project.orchestration/v1@9'
WHERE operating_skill_revision_id = 'forge.project.orchestration/v1@8';
