-- The Project Agent could record a validation result but was never told when.
--
-- `delivery_followup` fires as each Task completes, and the only instruction
-- attached to it was "evaluate readiness" -- so an Agent that read the state
-- correctly still answered by re-reporting the missing results it was supposed
-- to produce. Revision @7 names the trigger: when a milestone's Tasks are done,
-- its acceptance checks are the remaining work, and settling them comes before
-- readiness. It also states the baseline prerequisite, so a Project without an
-- approved baseline is reported as such instead of narrated as validated.
...
SELECT
    'forge.project.orchestration/v1@7', operating_skill_id, skill_key, 7,
...
',
    ...
    'ca6ae09e8b911a63cc97e50f3652382359d25381eb2f2378c4f4c48c49e3fb4b',
...
SET current_revision_id = 'forge.project.orchestration/v1@7',
    version = version + 1,
    updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')
WHERE id = 'forge.project.orchestration/v1'
  AND current_revision_id IS NOT 'forge.project.orchestration/v1@7';

UPDATE project_agent_binding
SET operating_skill_revision_id = 'forge.project.orchestration/v1@7'
WHERE operating_skill_revision_id = 'forge.project.orchestration/v1@6';
