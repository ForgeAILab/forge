...
async fn auditor_failure_dispatches_follow_up_executor_with_thread_reuse() {
    let repo_dir = common::TestDir::new("forge-review-fail-repo");
    let repo_path = setup_git_repo(repo_dir.path()).await;
    let default_branch = run_git(&repo_path, &["symbolic-ref", "--short", "HEAD"]);

    let workspaces_root = common::TestDir::new("forge-review-fail-workspaces");
...
async fn test_app(
    ...
    let web_dist_dir = common::TestDir::new("forge-review-fail-web");
