...
- **Provider entries:** each card shows provider type, editable display name, credential method, redacted account identity, endpoint, connection status, usage (which agents reference it and when it was last used), a live connection test, and reversible enable/disable plus rename/disconnect actions. Disabling preserves the credential and dependent agents, lists the affected agents before confirmation, and blocks new turns until re-enabled. Disconnecting remains the destructive credential-removal path and warns with the dependent agent list first; dependents become visibly unhealthy and are never silently rebound. CLI runtime cards show authentication availability, host, version, usage, a login-command hint, and the same reversible enable/disable control for that exact host/runtime pair.
...
- **Actions:** add/rename/enable/disable/disconnect a provider entry, enable or disable a CLI runtime, create and reversibly enable/disable an agent, publish a replacement profile on another entry, activate a profile revision, select exactly one Main identity/profile through `/account/main-agent`, and select exactly one Project identity/profile through `/projects/{id}/project-agent`. Source disable and Agent disable preserve bindings and configuration; they never silently replace an identity. Provider authorization, agent creation, and binding remain three separate operations.
...
- **CLI permission policy:** the shared policy selector offers `Supervised`, `Auto`, `Plan`, and an explicit `YOLO` choice for CLI-harness Task execution. `YOLO` uses the warning treatment and persistent help text stating that the CLI receives full host access without approval prompts. It never implies broader Forge authority: Main/Project Agent Chat filesystem denial, canonical scope, Task assignment/lease checks, and user-only approval, waiver, and release actions remain unchanged. The selector supports default, hover, active, visible focus, disabled, and selected-warning states at every breakpoint.
- **Activation summary:** every Agent detail renders a read-only `When this agent runs` section after its editable settings. It separates direct Chat messages, approved Project-creation handoffs, policy-admitted background Attention wakes, and Task workflow role dispatch. Each row states the required binding/assignment, whether the path uses the Project wake budget, and the current bound scopes. Raw durable event names live in a collapsed `Technical event names` disclosure; the visible summary never implies that a persisted `subscriptions` array is a working per-event filter. Disabled or unhealthy Agents remain listed with their bindings intact, but the section must state that new admissions are refused until effective availability returns.
...
### Reconciliation decision card

- **Structure:** lead with the concrete effect on the Project (for example, “Replace the assigned Project Agent” or “Replace this Task”) and exactly two visible decisions: `Accept` and `Reject`. Internal reconciliation states, record identifiers, revisions, digests, and affected paths live only inside a collapsed `Technical details` disclosure.
- **Semantics:** `Accept` applies the server-recommended replacement and resumes only the affected work. `Reject` keeps the current governing record and permanently rejects that proposed change; a temporary defer is labeled separately and never presented as rejection.
- **States:** pending, saving, resolved, stale-version conflict, unavailable, and missing-recommendation states use truthful copy and preserve the visible effect. A successful decision removes the active controls immediately and shows a compact outcome.
- **Accessibility:** the effect is a heading, both decisions are native buttons, pending controls are disabled, errors are announced, and the technical disclosure uses native `details`/`summary` keyboard behavior.
