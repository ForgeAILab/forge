...
- **Provider entries:** each card shows provider type, editable display name, credential method, redacted account identity, endpoint, connection status, usage (which agents reference it and when it was last used), a live connection test, and reversible enable/disable plus rename/disconnect actions. A failed provider call shows an Error or Backing off badge, a redacted reason, and the local retry time where applicable. The icon action runs a live connection test and refreshes that state. Auth errors remain blocked until a successful test. Disabling preserves the credential and dependent agents, lists the affected agents before confirmation, and blocks new turns until re-enabled. Disconnecting remains the destructive credential-removal path and warns with the dependent agent list first; dependents become visibly unhealthy and are never silently rebound. CLI runtime cards show authentication availability, host, version, usage, a login-command hint, and the same reversible enable/disable control for that exact host/runtime pair.
